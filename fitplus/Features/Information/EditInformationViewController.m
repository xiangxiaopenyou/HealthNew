...
@property (weak, nonatomic) IBOutlet UIView *heightPickerContainView;
@property (weak, nonatomic) IBOutlet UIView *agePickerContainView;
@property (weak, nonatomic) IBOutlet UIView *weightPickerContainView;
...
- (void)viewDidLoad {
    ...
    if (![self isNetworking]) {
        if ([[NSUserDefaults standardUserDefaults] integerForKey:UserHeight] == 0) {
            _informationModel.height = @"170";
        } else {
        ...
        if ([[NSUserDefaults standardUserDefaults] integerForKey:UserWeight] == 0) {
            _informationModel.weight = @"60";
        } else {
            _informationModel.weight = [[NSUserDefaults standardUserDefaults] valueForKey:UserWeight];
        }
        _informationModel.age = [[NSUserDefaults standardUserDefaults] stringForKey:UserAge];
...
- (void)saveClick {
    ...
    [[[RBBlockAlertView alloc] initWithTitle:@"" message:@"确定保存吗？" block:^(NSInteger buttonIndex) {
        if (buttonIndex == 1) {
            ...
            [[NSUserDefaults standardUserDefaults] setObject:_informationModel.age forKey:UserAge];
    ...
    switch (indexPath.section) {
        case 0:{
            ...
            if (indexPath.row == 0) {
                ...
                tempLabel.text = [NSString stringWithFormat:@"%@周岁", _informationModel.age];
            }
        }
            break;
        case 1:{
    ...
    switch (indexPath.section) {
        case 0:{
            if (indexPath.row == 0) {
                ...
                _agePickerContainView.hidden = NO;
                _weightPickerContainView.hidden = YES;
                _heightPickerContainView.hidden = YES;
                [_agePickerView selectRow:[_informationModel.age integerValue] - 1 inComponent:0 animated:YES];
            }
        }
            break;
        case 1:{
            ...
            [_signTextField resignFirstResponder];
            if (indexPath.row == 0) {
                _agePickerContainView.hidden = YES;
                _heightPickerContainView.hidden = NO;
                _weightPickerContainView.hidden = YES;
                ...
                _agePickerContainView.hidden = YES;
                _heightPickerContainView.hidden = YES;
                _weightPickerContainView.hidden = NO;
...
- (void)hidePickerView {
    [_heightPickerContainView setHidden:YES];
    [_weightPickerContainView setHidden:YES];
    [_agePickerContainView setHidden:YES];
}
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component{
    if (pickerView == _heightPickerView || pickerView == _agePickerView) {
        _indexRow2 = row;
    } else {
        if (component == 0) {
    ...
    if (pickerView == _heightPickerView) {
        ...
        _informationModel.height = [NSString stringWithFormat:@"%@",@(_indexRow2 + 100)];
    }
    if (pickerView == _weightPickerView) {
        if (!_indexRow2) {
            _indexRow2 = [_informationModel.weight integerValue] - 20;
        }
        _informationModel.weight = [NSString stringWithFormat:@"%@.%@",@(_indexRow2 +20), @(_indexRow)];
    }
    if (pickerView == _agePickerView) {
        if (!_indexRow2) {
            _indexRow2 = [_informationModel.height integerValue] - 1;
        }
        _informationModel.age = [NSString stringWithFormat:@"%@", @(_indexRow2 + 1)];
    }
    [_informationTableView reloadData];
}
- (BOOL)isNetworking {
...
- (IBAction)finishPickButtonClick:(id)sender {
    [self hidePickerView];
}
