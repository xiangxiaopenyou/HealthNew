...
@property (copy, nonatomic) DeleteTrend deleteTrend;
@end
            ...
            [[[RBBlockActionSheet alloc] initWithTitle:nil clickBlock:^(NSInteger buttonIndex) {
                if (buttonIndex == 3) {
                    ...
                    [[[RBBlockAlertView alloc] initWithTitle:nil message:@"确定要加入黑名单吗?" block:^(NSInteger buttonIndex) {
                        if (buttonIndex == 1) {
                            [self addBlackListUser:_selectedCommentModel.firstUserId];
                        }
                    } cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil] show];
                }
...
- (void)clockInCommentReport {
    ...
    [RecommendationModel clockinReport:_selectedCommentModel.commentid type:typeString handler:^(id object, NSString *msg) {
        if (msg) {
            [[[RBBlockAlertView alloc] initWithTitle:@"提示" message:@"举报失败" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil] show];
        } else {
            [[[RBBlockAlertView alloc] initWithTitle:@"提示" message:@"举报成功" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil] show];
            [self fetchClockInComments];
        }
    }];
}
    ...
    if ([_model[@"flag"] integerValue] == 5) {
        ...
        [[[RBBlockActionSheet alloc] initWithTitle:nil clickBlock:^(NSInteger buttonIndex) {
            if (buttonIndex == 0) {
                ...
                [[[RBBlockAlertView alloc] initWithTitle:nil message:@"确定要加入黑名单吗?" block:^(NSInteger buttonIndex) {
                    if (buttonIndex == 1) {
                        [self addBlackListUser:_model[@"userid"]];
                    }
                } cancelButtonTitle:@"取消" otherButtonTitles:@"确定", nil] show];
            }
        } cancelButtonTitle:@"取消" destructiveButtonTitle:@"举报不良信息" otherButtonTitles:@"取消关注", @"加入黑名单", nil] showInView:self.view];
    } else {
        [[[RBBlockActionSheet alloc] initWithTitle:nil clickBlock:^(NSInteger buttonIndex) {
            if (buttonIndex == 0) {
                [[[RBBlockAlertView alloc] initWithTitle:nil message:@"确定要举报吗?" block:^(NSInteger buttonIndex) {
...
- (void)reportClockIn {
    [RecommendationModel clockinReport:_clockinId type:@"trends" handler:^(id object, NSString *msg) {
        if (msg) {
            ...
            //[[[RBBlockAlertView alloc] initWithTitle:@"提示" message:@"举报成功" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil] show];
            [[[RBBlockAlertView alloc] initWithTitle:@"提示" message:@"举报成功" block:^(NSInteger buttonIndex) {
...
- (void)addBlackListUser:(NSString *)userId {
    [BlackUserModel addBlack:userId handler:^(id object, NSString *msg) {
        if (!msg) {
            [[[RBBlockAlertView alloc] initWithTitle:@"提示" message:@"添加到黑名单成功" block:^(NSInteger buttonIndex) {
                [[NSNotificationCenter defaultCenter] postNotificationName:ClockInOverNotificationKey object:nil];
                [self.navigationController popViewControllerAnimated:YES];
            } cancelButtonTitle:@"确定" otherButtonTitles:nil, nil]  show];
        } else {
            [[[RBBlockAlertView alloc] initWithTitle:@"提示" message:@"添加到黑名单失败" delegate:nil cancelButtonTitle:@"确定" otherButtonTitles:nil, nil] show];
        }
    }];
}
