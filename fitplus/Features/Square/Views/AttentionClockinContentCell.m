...
- (void)setupCellViewWithDic:(ClockInDetailModel *)data {
    ...
    if (data.clockinType == 2) {
        ...
        _energyTipOneImage.hidden = YES;
        _foodEnergyLabel.text = nil;
    }

    _commentNumberLabel.text = [NSString stringWithFormat:@"%@", @(data.commentNumber)];
    ...
    _likeNumberLabel.text = [NSString stringWithFormat:@"%@", @(data.likeNumber)];
...
- (IBAction)likeButtonClick:(id)sender {
    if (_model.isfavorite == 2) {
        ...
        _likeNumberLabel.text = [NSString stringWithFormat:@"%@", @(_model.likeNumber)];
    } else {
        _model.isfavorite = 2;
        _model.likeNumber -= 1;
        _likeImage.image = [UIImage imageNamed:@"dislike"];
