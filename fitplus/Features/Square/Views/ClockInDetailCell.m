...
@property (weak, nonatomic) IBOutlet UIImageView *energyTipImage;
...
- (void)setupCellViewWithModel:(NSDictionary *)data {
    ...
    if ([data[@"trendsportstype"] integerValue] == 2) {
    ...
    } else if ([data[@"trendsportstype"] integerValue] == 2) {
        ...
        _energyTipImage.hidden = YES;
        _energyLabel.text = nil;
...
- (IBAction)likeButtonClick:(id)sender {
    if (_isLike) {
        ...
        _likeNumber -= 1;
        _likeNumberLabel.text = [NSString stringWithFormat:@"%@", @(_likeNumber)];
    } else {
        _isLike = YES;
        _likeImage.image = [UIImage imageNamed:@"like"];
