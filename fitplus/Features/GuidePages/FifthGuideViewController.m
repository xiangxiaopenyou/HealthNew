...
@interface FifthGuideViewController ()<UIPickerViewDataSource, UIPickerViewDelegate, UIScrollViewDelegate>
...
@property (weak, nonatomic) IBOutlet UIView *weightContainView;
@property (weak, nonatomic) IBOutlet UIView *heightContainView;
@property (weak, nonatomic) IBOutlet UIView *ageContainView;
...
- (void)viewDidLoad {
    ...
    NSString *nickname = [[NSUserDefaults standardUserDefaults] stringForKey:UserName];
    _tipLabel.text = [NSString stringWithFormat:@"hi, %@, 让我们简单了解一下您的身体状况,方便教练给您制定方案", nickname];
    if ([[NSUserDefaults standardUserDefaults] objectForKey:UserHeight]) {
        _userHeight = [[NSUserDefaults standardUserDefaults] integerForKey:UserHeight];
        _heightLabel.text = [NSString stringWithFormat:@"%@", @(_userHeight)];
    }
    if ([[NSUserDefaults standardUserDefaults] objectForKey:UserWeight]) {
    ...
    if ([[NSUserDefaults standardUserDefaults] objectForKey:UserAge]) {
        _userAge = [[NSUserDefaults standardUserDefaults] integerForKey:UserAge];
        _ageLabel.text = [NSString stringWithFormat:@"%@", @(_userAge)];
    }
...
- (void)viewDidAppear:(BOOL)animated {
    ...
    [_agePickerView selectRow:20 inComponent:0 animated:NO];
}
#pragma mark - Request
...
- (NSString*)pickerView:(UIPickerView *)pickerView titleForRow:(NSInteger)row forComponent:(NSInteger)component {
    if (pickerView == _pickerView) {
        return [NSString stringWithFormat:@"%@", @(row + 100)];
    } else if (pickerView == _weightPickerView) {
        switch (component) {
            case 0:
                return [NSString stringWithFormat:@"%@", @(row + 20)];
                break;
            case 1:
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    if (pickerView == _pickerView) {
        ...
        if (weightInt2 != 0) {
            _weightLabel.text = [NSString stringWithFormat:@"%@.%@", @(weightInt1 + 20), @(weightInt2)];
        } else {
            _weightLabel.text = [NSString stringWithFormat:@"%@", @(weightInt1 + 20)];
        }
        _userWeight = [_weightLabel.text floatValue];
    } else {
        NSInteger ageInt = [pickerView selectedRowInComponent:0];
        _userAge = ageInt + 1;
        _ageLabel.text = [NSString stringWithFormat:@"%@", @(_userAge)];
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    _heightContainView.hidden = YES;
    _weightContainView.hidden = YES;
    _ageContainView.hidden = YES;
}
...
- (IBAction)heightButtonClick:(id)sender {
    _heightContainView.hidden = NO;
    _weightContainView.hidden = YES;
    _ageContainView.hidden = YES;
}
- (IBAction)weightButtonClick:(id)sender {
    _weightContainView.hidden = NO;
    _heightContainView.hidden = YES;
    _ageContainView.hidden = YES;
}
...
- (IBAction)submitButtonClick:(id)sender {
    ...
    if (_userAge == 0) {
        [[[RBBlockAlertView alloc] initWithTitle:@"警告" message:@"记得选择年龄哦~" block:^(NSInteger buttonIndex) {
        } cancelButtonTitle:@"确定" otherButtonTitles:nil, nil] show];
        return;
    }
    if (_userBody == 0) {
    ...
    [[NSUserDefaults standardUserDefaults] setObject:@(_userAge) forKey:UserAge];
...
//            _courseArray = [model.result mutableCopy];
            ...
            resultViewController.isInformationViewIn = YES;
...
- (IBAction)finishButtonClick:(id)sender {
    UIButton *button = (UIButton *)sender;
    if (button.tag == 999) {
        NSInteger weightInt1 = [_weightPickerView selectedRowInComponent:0];
        NSInteger weightInt2 = [_weightPickerView selectedRowInComponent:1];
        if (weightInt2 != 0) {
            _weightLabel.text = [NSString stringWithFormat:@"%@.%@", @(weightInt1 + 20), @(weightInt2)];
        } else {
            _weightLabel.text = [NSString stringWithFormat:@"%@", @(weightInt1 + 20)];
        ...
        NSInteger heightInt = [_pickerView selectedRowInComponent:0];
        _userHeight = heightInt + 100;
        _heightLabel.text = [NSString stringWithFormat:@"%@", @(_userHeight)];
    } else {
        NSInteger ageInt = [_agePickerView selectedRowInComponent:0];
        _userAge = ageInt + 1;
        _ageLabel.text = [NSString stringWithFormat:@"%@", @(_userAge)];
    }
    _heightContainView.hidden = YES;
