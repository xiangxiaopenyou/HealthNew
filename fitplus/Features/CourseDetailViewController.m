...
- (void)viewDidLoad {
    ...
    _topBackgroundView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, 269)];
    _topBackgroundView.backgroundColor = [UIColor colorWithWhite:0 alpha:0.4];
    ...
    if (_isJoin) {
        [_bottomButton setTitle:[NSString stringWithFormat:@"开始第%@天训练", @([_courseModel.couserDayEnNum integerValue] + 1)] forState:UIControlStateNormal];
    } else {
...
- (void)addTableHeaderView {
    ...
    _daysLabel.text = [NSString stringWithFormat:@"%@天", @(_courseModel.courseDays)];
    ...
    if ([_courseModel.couserDayEnNum integerValue] == _courseModel.courseDays) {
        ...
        NSString *day = [NSString stringWithFormat:@"%@", @([_courseModel.couserDayEnNum integerValue] + 1)];
        NSString *targetDayString = [NSString stringWithFormat:@"今日目标%@/%@天", @([_courseModel.couserDayEnNum integerValue] + 1), @(_courseModel.courseDays)];
...
- (void)fetchCourseDetail {
    [CourseDetailModel fetchCourseDetailWith:_courseId handler:^(id object, NSString *msg) {
        if (!msg) {
...
- (void)fetchCourseMember {
    [CourseTrendModel fetchCourseMember:_courseId limit:1 handler:^(id object, NSString *msg) {
        if (!msg) {
            _memberArray = [object[@"list"] mutableCopy];
        }
         [self fetchCourseTrends];
    }];
}
- (void)fetchCourseTrends {
    [CourseTrendModel fetchCourseTrends:_courseId limit:_limit handler:^(id object, NSString *msg) {
...
- (void)joinCourse {
    ...
    [CourseDetailModel joinCourseWith:_courseId dayId:dayIdString handler:^(id object, NSString *msg) {
        if (!msg) {
            _isJoin = YES;
            [self fetchCourseDetail];
        }
...
- (void)setupData:(CourseDetailModel *)model {
    if (_nowCourseDay == 0) {
        ...
        NSString *day = [NSString stringWithFormat:@"%@", @(_nowCourseDay)];
        NSString *targetDayString = [NSString stringWithFormat:@"今日目标%@/%@天", day, @(_courseModel.courseDays)];
        NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:targetDayString];
        [attributedString addAttribute:NSFontAttributeName value:[UIFont systemFontOfSize:25] range:NSMakeRange(4, [day length])];
        _targetDayLabel.attributedText = attributedString;
    }
    if (!_isJoin) {
        ...
        _courseTimeLabel.text = [NSString stringWithFormat:@"%@天", @(_courseModel.courseDays)];
        NSString *day = [NSString stringWithFormat:@"%@", @(_courseModel.courseDays)];
        NSString *targetDayString = [NSString stringWithFormat:@"%@天", @(_courseModel.courseDays)];
        NSMutableAttributedString *attributedString = [[NSMutableAttributedString alloc] initWithString:targetDayString];
        ...
        NSString *energyString = [NSString stringWithFormat:@"%@", @(_courseModel.total_calories)];
        ...
        if (actionArray.count == 0) {
            actionString = [NSString stringWithFormat:@"%@", @([_actionArray[0][@"resolve_list"] count])];
        } else {
            actionString = [NSString stringWithFormat:@"%@", @(actionArray.count)];
        }
        ...
        if (actionArray.count == 0) {
            energyString = [NSString stringWithFormat:@"%@", _actionArray[0][@"calories"]];
        } else {
        ...
        if (_nowCourseDay == 0) {
            ...
            [_bottomButton setTitle:[NSString stringWithFormat:@"开始第%@天训练", @(_nowCourseDay)] forState:UIControlStateNormal];
        }
    }
    [self setupHeaderView];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    if (_selectedType == 1) {
        if (indexPath.row == 0) {
            ...
            cell.courseMemberLabel.text = [NSString stringWithFormat:@"%@人", @(_courseModel.courseMember)];
            ...
            cell.energyLabel.text = [NSString stringWithFormat:@"%@千卡", @(tempModel.calorie)];
            return cell;
        }
    }
    else {
        if (_isOpen && _selectedIndexPath.section == indexPath.section && indexPath.row != 0) {
            ...
            if (timeInt / 60 > 0) {
                cell.actionTimeLabel.text = [NSString stringWithFormat:@"%@分%@秒", @(timeInt / 60), @(timeInt % 60)];
            } else {
                cell.actionTimeLabel.text = [NSString stringWithFormat:@"%@秒", @(timeInt % 60)];
            }
            return cell;
        } else {
...
//            } else {
           ...
           // }
            cell.courseDayNameLabel.text = [NSString stringWithFormat:@"第%@天：%@", @(indexPath.section + 1), tempDictionary[@"resolve_name"]];
            NSInteger timeInt = [tempDictionary[@"total_time"] integerValue];
            if (timeInt / 60 > 0) {
                cell.courseDaysTime.text = [NSString stringWithFormat:@"%@分%@秒", @(timeInt / 60), @(timeInt % 60)];
            } else {
                cell.courseDaysTime.text = [NSString stringWithFormat:@"%@秒", @(timeInt % 60)];
            }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (_selectedType == 1) {
        if (indexPath.row > 0) {
            CourseTrendModel *tempModel = _trendArray[indexPath.row - 1];
            MyHomepageViewController *homepageViewController = [[UIStoryboard storyboardWithName:@"Information" bundle:nil] instantiateViewControllerWithIdentifier:@"MyHomepageView"];
            homepageViewController.friendId = tempModel.userId;
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (y < 0) {
        ...
        _topBackgroundView.frame = frame;
    } else {
        frame.origin.y = - y;
        ...
        [[[RBBlockActionSheet alloc] initWithTitle:nil clickBlock:^(NSInteger buttonIndex) {
            if (buttonIndex == 0) {
                [[[RBBlockAlertView alloc] initWithTitle:@"提示" message:@"确定要退出训练吗？" block:^(NSInteger buttonIndex) {
                    if (buttonIndex == 1) {
                        [CourseModel exitCourse:_courseId handler:^(id object, NSString *msg) {
                            if (!msg) {
                                _isJoin = NO;
                                [self fetchCourseDetail];
                            } else {
...
- (void)bottomButtonClick {
    if (_isJoin) {
        VideoModel *tempModel = [_videoArray lastObject];
