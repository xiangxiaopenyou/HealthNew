...
+ (void)record:(NSArray *)goods calories:(CGFloat)calories image:(UIImage *)image tags:(NSArray *)tags content:(NSString *)content topicId:(NSString *)topicId handler:(RequestResultHandler)handler {
    NSMutableDictionary *record = [[[self class] buildRecordWithRecord:goods calories:calories tags:tags content:content topicId:topicId] mutableCopy];
    if (image) {
...
+ (NSDictionary *)buildRecordWithRecord:(NSArray *)goods calories:(CGFloat)calories tags:(NSArray *)tags content:(NSString *)content topicId:(NSString *)topicId {
    ...
    if (topicId && ![topicId isEqualToString:@""]) {
        [record setObject:topicId forKey:@"HotListId"];
    }
