...
+ (void)fetchDryListWithKindsId:(NSString *)kindsId page:(NSInteger)page handler:(RequestResultHandler)handler {
    [[FetchDryListRequest new] request:^BOOL(FetchDryListRequest *request) {
        request.kindsId = kindsId;
        request.page = page;
        return YES;
    } result:^(id object, NSString *msg) {
        if (msg) {
            ...
            LimitResultModel *tempModel = [[LimitResultModel alloc] initWithNewResult:object modelKey:@"list" modelClass:[DrysalteryModel new]];
...
+ (void)fetchRecentDryTime:(RequestResultHandler)handler {
    [[FetchRecentDrysalteryRequest new] request:^BOOL(id request) {
        return YES;
    } result:^(id object, NSString *msg) {
        if (msg) {
            !handler ?: handler(nil, msg);
